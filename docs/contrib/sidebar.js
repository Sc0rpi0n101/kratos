...
const toHref = (slug, node) => {
  ...
  let res = request('GET', `https://raw.githubusercontent.com/ory/${slug}/new-doc-nav/docs/docs/${node}.mdx`);
  if (res.statusCode === 404) {
    res = request('GET', `https://raw.githubusercontent.com/ory/${slug}/new-doc-nav/docs/docs/${node}.md`);
  }
...
const resolveRefs = (node) => {
  if (node !== null && typeof node == "object") {
    if (node['$slug']) {
      ...
      const res = request('GET', `https://raw.githubusercontent.com/ory/${slug}/new-doc-nav/docs/sidebar.json`);
